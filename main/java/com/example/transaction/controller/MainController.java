...
public class MainController {
    ...
    public String myPage(HttpServletRequest request, Model model) {
        ...
        List<Item> myItems = itemRepository.findBySellerId(user.getId()).stream()
                .filter(item -> item.getBuyerId() == null) // 아직 판매되지 않은 아이템만
                .collect(Collectors.toList());

        List<Item> purchasedItems = itemRepository.findByBuyerId(user.getId());

        model.addAttribute("user", user);
        model.addAttribute("itemList", myItems);        // 판매중인 내 아이템
        model.addAttribute("purchasedList", purchasedItems); // 내가 구매한 아이템
